...
export default function AdminRegistrationsPage() {
  ...
  const handleResendInvite = async (member: Member) => {
    if (!member.userId) {
      toast.error("No user account linked to this registration");
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/admin/users/${member.userId}/resend-invite`,
        {
          method: "POST",
        },
      );
              ...
              <TableRow key={member.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    ...
                    {/* Resend Invite Button - Only show for approved members with userId */}
                    {member.status === "approved" && member.userId && (
                      <Button
                        variant="ghost"
                        size="icon"
                    ...
                    {/* For pending registrations - Approve/Reject buttons */}
                    ...
                    {/* For rejected registrations - Allow reversing back to pending */}
                    {member.status === "rejected" && (
                      <Button
                        ...
                        disabled={isUpdating}
                        title="Reverse to Pending"
                      >
                        <RefreshCw className="h-4 w-4" />
                      </Button>
                    )}

                    {/* For approved registrations without userId - Show link to create user */}
                    {member.status === "approved" && !member.userId && (
                      <Button
                        variant="ghost"
                        size="sm"
                        ...
                        onClick={() => {
                          // Navigate to add user with pre-filled email
                          window.location.href = `/admin/users?email=${encodeURIComponent(member.email || "")}&name=${encodeURIComponent(member.firstName + " " + member.surname)}`;
                        }}
              ...
              <div className="flex gap-3 pt-4 border-t mt-2">
                ...
                {selectedMember.status === "rejected" && (
                  <Button
                    ...
                    onClick={() =>
                      handleStatusChange(selectedMember.id, "pending")
                    }
                    disabled={isUpdating}
                  >
                    <RefreshCw className="h-4 w-4" />
                    {isUpdating ? "Processing..." : "Reverse to Pending"}
                  </Button>
                )}
                ...
                {selectedMember.status === "approved" &&
                  !selectedMember.userId && (
                    <Button
                      className="flex-1 gap-2 bg-primary hover:bg-gray-700"
                      onClick={() => {
                        window.location.href = `/admin/users?email=${encodeURIComponent(selectedMember.email || "")}&name=${encodeURIComponent(selectedMember.firstName + " " + selectedMember.surname)}`;
                      }}
                    >
                      <UserPlus className="h-4 w-4" />
                      Create User Account
                    ...
                    {selectedMember.status === "approved" &&
                      !selectedMember.userId && (
                        <span className="block mt-1 text-xs text-amber-600">
                          No user account is linked. Click "Create User Account"
                          to invite this member.
                        </span>
                      )}
                  </p>
                </div>
